...
public class VentaDao {
    ...
    public int RegistrarVenta(Venta v){
        ...
        //int a = 0;
        String sql = "INSERT INTO ventas (cliente, vendedor, total, fecha, contabilizador) VALUES (?,?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public boolean ActualizarStock(double cant, int id){
...
//SELECT pr.id AS id_proveedor, pr.nombre AS nombre_proveedor, p.* FROM proveedor pr INNER JOIN productos p ON pr.id = p.proveedor ORDER BY p.id DESC"    
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario, String fechaV) {
        try {
            FileOutputStream archivo;
            //String url = FileSystemView.getFileSystemView().getDefaultDirectory().getPath();
            String url = FileSystemView.getFileSystemView().getDefaultDirectory().getPath() + "\\Reportes_SistemaVenta\\Hist_Ventas\\";
            //String url = "C:\\Users\\Mario\\Documents";
            File salida = new File(url +  "venta_"+ idventa +".pdf");
            ...
            PdfPCell c3 = new PdfPCell(new Phrase("P. KG/U", negrita));
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    double subTotal = rs.getDouble("cantidad") * rs.getDouble("precio");
    ...
    //public void pdfCorteCaja(int folioC, String vandedorC, double totalC, String fechaC) {
    public void pdfCorteCaja(int folioC) {
        try {
            FileOutputStream archivo;
            String url = FileSystemView.getFileSystemView().getDefaultDirectory().getPath() + "\\Reportes_SistemaVenta\\Hist_CortesCaja\\";
            //String url = "C:/Users/Mario/Documents/";
            //String url = "dist/Reportes/Hist_CortesCaja/";
            File salida = new File(url + "CorteDeCaja_Folio_"+ folioC + ".pdf");
            archivo = new FileOutputStream(salida);
            Document doc = new Document();
            PdfWriter.getInstance(doc, archivo);
            doc.open();
            ...
            //Corte Caja
            Paragraph Corte = new Paragraph();
            Corte.add(Chunk.NEWLINE);
            Corte.add("DATOS DEL CORTE DE CAJA" + "\n\n");
            doc.add(Corte);

            Font negrita = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLD, BaseColor.BLUE);
            
            PdfPTable CorteCaja = new PdfPTable(4);
            CorteCaja.setWidthPercentage(100);
            CorteCaja.getDefaultCell().setBorder(0);
            float[] columnWidthsCliente = new float[]{20f, 25f, 25f, 50f};
            CorteCaja.setWidths(columnWidthsCliente);
            CorteCaja.setHorizontalAlignment(Element.ALIGN_LEFT);
            
            PdfPCell folio = new PdfPCell(new Phrase("FOLIO", negrita));
            ...
            folio.setBorder(Rectangle.NO_BORDER);
            vendedor.setBorder(Rectangle.NO_BORDER);
            total.setBorder(Rectangle.NO_BORDER);
            fecha.setBorder(Rectangle.NO_BORDER);
            
            CorteCaja.addCell(folio);
            CorteCaja.addCell(vendedor);
            CorteCaja.addCell(total);
            CorteCaja.addCell(fecha);
            
            String prove = "SELECT * FROM cortecaja WHERE folio = ?";
            try {
                ps = con.prepareStatement(prove);
                ps.setInt(1, folioC);
                rs = ps.executeQuery();
                if (rs.next()) {
                    CorteCaja.addCell(rs.getString("folio"));
                    CorteCaja.addCell(rs.getString("vendedor"));
                    CorteCaja.addCell("$ " + rs.getString("total"));
                    CorteCaja.addCell(rs.getString("fecha") + "\n\n");
                } 
            } catch (SQLException e) {
                System.out.println(e.toString());
            }
